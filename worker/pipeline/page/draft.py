...
DRAFT_SYSTEM = (
    ...
    "- Mermaid fence contract:\n"
    "  - Every Mermaid diagram MUST be a complete fenced block: opening "
    "```mermaid, diagram body, then closing ```\n"
    "  - Never leave a Mermaid fence unclosed\n"
    "  - Put source annotations after the closing fence, never inside the "
    "Mermaid block\n"
    "  - Do not include Markdown headings, prose, source annotations, tables, "
    "bullets, or inline code inside Mermaid blocks\n"
    "  - In stateDiagram-v2, every composite state opened with `state Name {` "
    "MUST be closed with `}` before the Mermaid fence closes\n"
    "  - Before finishing, verify every ```mermaid opening fence has a matching "
    "closing ``` fence before the next Markdown heading, source annotation, "
    "table, or paragraph\n"
