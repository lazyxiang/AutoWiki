...
async def test_get_job_found(client):
    ...
    async with get_session(str(cfg.database_path)) as s:
        repo = Repository(id="r2", owner="a", name="b", status="ready", platform="github")
        job = Job(id="job-found", repo_id="r2", type="full_index", status="queued", progress=0)
        s.add(repo)
        s.add(job)
        await s.commit()
...
async def test_ws_job_progress(tmp_path):
    ...
    async with get_session(str(tmp_path / "test.db")) as s:
        repo = Repository(id="r1", owner="a", name="b", status="ready", platform="github")
