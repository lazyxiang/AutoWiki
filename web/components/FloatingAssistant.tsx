...
export function FloatingAssistant({ repoId }: FloatingAssistantProps) {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    // B5: Research mode removed (issue #43); always route to Fast Report / chat.
    const targetPath =
      ...
      <div className="z-20 w-full rounded-2xl backdrop-blur-md transition-shadow focus-within:ring-2 focus-within:ring-indigo-500/20 bg-white/70 border border-indigo-300/20 shadow-lg shadow-indigo-500/20 p-2 flex flex-col gap-1.5">
        <div className="flex items-center gap-1 px-1">
          <Button
            variant="ghost"
            size="sm"
            className="h-8 gap-1.5 text-xs font-medium rounded-lg px-3 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700"
          >
            <MessageSquare className="h-3.5 w-3.5" />
            Fast Report
          </Button>
          {/* B5: Research button hidden — Deep Research disabled (issue #43). */}
        </div>
