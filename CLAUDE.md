...
4. **Keyword Index** (`worker/pipeline/retrieval/chunking.py` + `keyword_index.py`) — entity-aware chunking → `KeywordIndex` (BM25 via `rank_bm25`); rebuilt in memory each run. Stage 4's FAISS+embedding path was removed in Layer B (issue #43); the indexing pipeline no longer requires an embedding API key.
...
- **Worker job entrypoints**: `worker/jobs.py` is the ARQ compatibility facade. Full-index orchestration lives in `worker/index/full.py`; refresh orchestration lives in `worker/index/refresh.py`; Fast Report orchestration and retrievers live in `worker/fast_report/jobs.py`; Deep Research orchestration lives in `worker/research/jobs.py`.
...
- **Planner page-centric file selection**: Phase 2 (`_select_files`) selects 5–8 representative files per page (max 10) rather than assigning every file to one page. `_prefilter_candidates` scores all repo files per page and passes the top **40** candidates to the LLM (raised from 25 in PR #40, issue #39, to give Phase 2 enough breadth on medium/large repos — see `docs/spec/claude/2026-04-29-wiki-page-quality-redesign.md:37`). Orphan enforcement removed — unselected files are intentionally omitted.
- **Planner fallback semantics**: when `_select_files` exhausts retries, `_heuristic_select_files` preserves valid pages from the partial LLM result and fills the remainder via scoring. There is no secondary assignment fallback; unselected files are intentionally omitted.
